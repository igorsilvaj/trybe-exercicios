...
const input1 = document.getElementById('phrases-input');
const list1 = document.getElementById('phrases-list');
...
function addPhraseToSessionStorage() {
  ...
  const phraseText = input1.value;
  oldList.push(phraseText);
  sessionStorage.setItem('phrases', JSON.stringify(oldList));
  insertPhraseInDOM();
}

button1.addEventListener('click', addPhraseToSessionStorage);